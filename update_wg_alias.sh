...
[ -f "$ASN_FILE" ] || { echo "ASN-Datei fehlt: $ASN_FILE" >&2; exit 1; }

# ASNs (case-insensitiv)
grep -i -oE 'AS[0-9]+' "$ASN_FILE" | sort -u > "$ASN_TMP" || true
# IPv4 single IPs
grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}(/[0-9]{1,2})?' "$ASN_FILE" \
  | grep -v '/' | sort -u > "$IP4_TMP" || true
# IPv6 single IPs (rudimentär; Zeilen mit ':' & Hex, ohne Slash)
grep -iEo '([0-9a-f:]+)' "$ASN_FILE" | grep ':' | grep -v '/' | sort -u > "$IP6_TMP" || true
# IPv4 CIDRs
grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}' "$ASN_FILE" | sort -u > "$CIDR4_TMP" || true
# IPv6 CIDRs
grep -iEo '([0-9a-f:]+)/[0-9]{1,3}' "$ASN_FILE" | grep ':' | sort -u > "$CIDR6_TMP" || true
